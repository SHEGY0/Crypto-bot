...
FIAT_CURRENCIES = ["RUB", "USD", "EUR", "UAH", "KZT"]
...
async def process_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("❌ Введите корректное число, например: <code>100</code>", parse_mode="HTML")
        return

    data = await state.get_data()
    from_currency = data["from_currency"]
    to_currency = data["to_currency"]
    to_fiat = to_currency in FIAT_CURRENCIES
    ...
    try:
        ...
        await state.update_data(
            amount=amount, to_amount=to_amount, rate=rate,
            fee_usd=fee_usd, amount_usd=amount_usd, to_fiat=to_fiat
        )

        if to_fiat:
            # Крипто → Фиат: кошелёк не нужен, сразу подтверждение
            ...
            await message.answer(
                ...
                f"⚠️ После подтверждения получите адрес для отправки средств.",
                parse_mode="HTML",
                reply_markup=confirm_exchange_keyboard()
            )
        else:
            # Крипто → Крипто: спрашиваем кошелёк получателя
...
async def process_wallet(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ <b>Подтвердите заявку:</b>\n\n"
        f"▶️ Отдаёте: <b>{data['amount']} {data['from_currency']}</b>\n"
        f"◀️ Получаете: <b>{data['to_amount']:.6f} {data['to_currency']}</b>\n"
        f"💸 Комиссия: ${data['fee_usd']:.2f}\n"
...
async def confirm_exchange(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"⚠️ Отправляйте <b>точную сумму!</b>\n\n"
        f"После отправки пришлите <b>хеш транзакции (TXID)</b> 👇",
        parse_mode="HTML"
    )
    await callback.answer()
...
async def process_hash(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ <b>Хеш принят!</b>\n\n"
        f"Теперь укажите реквизиты для выплаты рублей.\n\n"
        f"📱 Введите <b>номер телефона</b> для СБП:\n"
...
async def process_name(message: Message, state: FSMContext):
    ...
    try:
        for admin_id in ADMIN_IDS:
            await message.bot.send_message(
                ...
                f"👤 @{message.from_user.username} (ID: {message.from_user.id})\n\n"
                f"💱 {amount} {from_currency} → {to_amount:,.2f} {to_currency}\n"
                f"💸 Комиссия: ${fee_usd:.2f}\n\n"
                f"🔗 <b>TXID:</b>\n<code>{tx_hash}</code>\n\n"
                f"🏦 <b>Реквизиты СБП:</b>\n"
    ...
    await message.answer(
        ...
        f"📱 {phone} · {bank}\n"
        f"👤 {full_name}\n\n"
        f"⏱ <b>Выплата в течение 5-30 минут</b> после подтверждения транзакции.\n\n"
